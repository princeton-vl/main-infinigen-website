...
Alternatively, we support generating dense annotations from Blender's built-in render passes.
:::
...
If you want CUDA optimizations, simply run install.sh on a device with a CUDA-enabled graphics card.
:::caution Troubleshooting

If you want to run without OpenGL-based groundtruth compilation and without CUDA terrain even on a CUDA-enabled machine, run
